...
'A__C': A로 시작, 아무 문자 둘, C로 끝	ABDC, ABCC 매칭
*/

-- Q1. 이름이 S로 시작하는 사원을 출력해보자.
SELECT ename
FROM emp
WHERE ename LIKE 'S%';   -- S로 시작하는 사원

-- Q2. 이름이 T로 끝나는 사원을 출력해보자.
SELECT ename
FROM emp
...
SELECT ename
FROM emp
WHERE ename LIKE '%TT';   -- T로 끝나는 사원

SELECT ename
FROM emp
WHERE ename LIKE '%_T';   -- T로 끝나는 사원

-- Q3. 이름에 A가 포함된 사원을 출력해보자.
-- 결과: 이름 앞뒤에 A가 포함된 경우가 전부 출력된다.
SELECT ename
FROM emp
...
-- Q2. TH로 끝나는 사원의 이름을 출력해보자
SELECT ename FROM emp WHERE ename REGEXP 'TH$';

-- Q3. 이름에 L, M, N이 들어간 사원을 출력해보자.
SELECT ename FROM emp WHERE ename REGEXP '[LMN]';

-- Q4. 이름이 4글자인 사원을 출력하자.
-- 시작과 끝을 명시하고 아무 글자나 4글자를 출력한다.
...
REGEXP_INSTR(검색 대상, '정규식', 시작위치, 검색 횟수/위치): 위치리턴
*/
...
-- 영문자를 소문자로 매칭 [a-z]+ 를 X로 3번째 매치를 변경해줘
SELECT REGEXP_REPLACE('abc def ghi', '[a-z]+', 'X', 1, 3) as res;

-- 영문자를 소문자로 매칭 [a-z]+ 를 X로 2번째 매치를 변경해줘
SELECT REGEXP_REPLACE('abc def ghi', '[a-z]+', 'X', 1, 2) as res;

-- 영문자를 소문자로 매칭 [a-z]+ 를 X로 변경해줘
SELECT REGEXP_REPLACE('abc def Gabcd', '[a-z]+', 'X' ,1,1) as result01;

SELECT REGEXP_LIKE('justalnums', '[[:alnum:]]+') as res02; 

-- SHOW VARIABLES LIKE 'character_set%'; 을 확인하여 인코딩 내용 보기
...
-- 테이블 정보 확인시 나오는 ci : case insensitive 
...
전체 컬럼에서 대하여 추가적인 요약 정보를 단계적으로 만들어 준다  _MYSQL에는 함수가 없음 x 
*/
/*
...
-- ENAME, COMM 컬럼 기준으로 그룹화한 후 각 그룹마다 COMM의 합계 SUM(COMM)을 계산
...
/*	 실행 순서 (ENAME, COMM) -> 정상 그룹,
		...
		GROUPING(ENAME) : 1이면 ENAME이 NULL, 0이면 실제 데이터
