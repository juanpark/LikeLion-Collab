...
public class PoolExample_test {
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= 5; i++) {
			...
			// 디버깅: 자세하게 살펴보기!!!
			executor.submit(() -> {
			    System.out.println(Thread.currentThread().getName() + " → 작업 " + taskNum + " 시작");
			    try {
			        Thread.sleep(100); // simulate work
			    } catch (InterruptedException e) {
			        e.printStackTrace();
			    }
			    System.out.println(Thread.currentThread().getName() + " → 작업 " + taskNum + " 종료");
			});
		}
