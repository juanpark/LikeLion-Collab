...
public class JDBCTemplate {
    ...
    static { // static 생성자 역할 -> 정적 메모리 확보를 제일 먼저 실행되고 바인드된다. 
        try (InputStream is = JDBCTemplate.class.getClassLoader().getResourceAsStream("db.properties")) {
            ...
            prop.load(is); // Reads a property list (key and element pairs) from the input byte stream.
    ...
    public static Connection getConnection() {
        try {
            ...
            return conn; // return은 메서드를종료하는 키워드 
    ...
    public static void commit(Connection conn) {
        try { if (conn != null) conn.commit(); } catch (Exception e) {e.printStackTrace();}
    }
